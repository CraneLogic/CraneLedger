...
router.post('/:id/balance', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error recording balance payment', { error: error.message });
    return res.status(error.statusCode || 500).json({ error: error.message });
  }
});
...
router.post('/:id/payout', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error recording supplier payout', { error: error.message });
    return res.status(error.statusCode || 500).json({ error: error.message });
  }
});
...
router.get('/external/:entityId/:externalBookingId', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error getting booking by external ID', { error: error.message });
    return res.status(error.statusCode || 500).json({ error: error.message });
  }
});
